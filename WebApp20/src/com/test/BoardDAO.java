	...
	*/
	...
	public int getDataCount(String searchKey, String searchValue)
	{
		...
		try
		{
			...
			sql = "SELECT COUNT(*) AS COUNT FROM TBL_BOARD WHERE " + searchKey + " LIKE ?"; 			//+ "WHERE ? LIKE ?"; -> 이렇게 해도 되나 문자열을 나타내는 ''의 오류가 있을 수 있음
	...
	public List<BoardDTO> getLists(int start, int end)
	{
		...
		try
		{
			...
			//sql += "	FROM ";
			//sql += "	(";
			//sql += "    	SELECT NUM, NAME, SUBJECT, HITCOUNT, TO_CHAR(CREATED, 'YYYY-MM-DD') AS CREATED --CREATED - 세션 설정에 따라서 2023/12/18, 2023.12.18로 보이기도 함";
			//sql += "    	FROM TBL_BOARD";
