...
%>
<%
	// 이전 페이지(최초 페이지를 list.jsp로 해놨고 번호를 눌러도 list.jsp로 오기 때문에 많다. 뭐라고 정의하기 어렵다.)로부터 넘어온 게시물 번호 수신
	...
	int currentPage=1; 			// null인 경우 1로 반환
	...
	if (searchKey != null)			//-- 검색 기능을 통해 이 페이지가 요청되었을 경우
	{
		...
		if (request.getMethod().equalsIgnoreCase("GET")) 	// 요청 방식이 GET 방식이라면~
		{
			// 디코딩 처리
	...
	//-- 현재 페이지가 1페이지에 있을 경우(currentPage=1)
	...
	// 검색값이 존재한다면...
	if (!searchValue.equals("")) //검색값이 비어있는 것이 아니라면
	{
			...
			<form action="" name="searchForm" method="post">
				<select name="searchKey" class="selectFiled">
					...
					<option value="content">내용</option>
					 ...
					 else if (searchKey.equals("content")) // 수시한 searchKey 가 content 라면
					 {
					 %>
					 	<option value="subject">제목</option>
						<option value="name">작성자</option>
					 ...
					 else		// 수시한 searchKey 가 subject 이거나... 없으면...
					 {
					 %>
					 	<option value="subject">제목</option>
						<option value="name">작성자</option>
