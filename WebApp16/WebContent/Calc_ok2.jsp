...
<!-- ob.su1 = xxx; 이렇게 하면 private 속성이므로 원래는 에러 뜨는데 ③으로 -->        
<jsp:setProperty property="su1" name="ob"/>
<!-- ① request.getParameter()가 주석처리 되어있음에도 알아서 처리 해줌 → 매개변수를 전달하기 위한 데이터 수신 처리 -->
<!-- ② Integer.parseInt() 처리 → 매개변수를 전달하기 위한 형 변환 처리도 알아서 해줌 -->
<!-- ③ ob.su1 → ob.setSu1() 호출 처리도 해준다. -->
<!-- ==> 최종적으로 ob → Calc 기반의 ob 객체의 su1 속성에
         이전 페이지로부터 넘겨받은 데이터를 적절한 타입(형)으로 전달해준다. -->
